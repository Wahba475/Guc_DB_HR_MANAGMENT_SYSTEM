...
export default function AnnualApprovalPage() {
  ...
  const fetchPendingAnnual = async () => {
    try {
      const res = await axios.get(
        "http://localhost:3000/academic/leaves/annual/pending",
      ...
      console.log(err);
      toast.error("Failed to load pending annual leaves.");
    }
  };
  ...
  const handleAction = async (request_ID, action) => {
    try {
      await axios.post(
        "http://localhost:3000/academic/leaves/annual/review",
        { request_ID, action },
        { headers: { Authorization: `Bearer ${token}` } }
      );

      const messages = {
        approve: "Annual leave approved successfully.",
        reject: "Annual leave rejected successfully."
      };
    ...
    <div className="grid grid-cols-[250px_1fr] h-screen bg-gray-50">
      ...
      <div className="p-10">
        <h1 className="text-3xl font-bold mb-6 text-emerald-800">
          Pending Annual Leave Approvals
        </h1>

        {requests.length === 0 ? (
          <p className="text-gray-600 mt-10">No pending annual leaves.</p>
        ) : (
